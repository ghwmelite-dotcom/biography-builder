...
export default function BannerVersionsDialog({ open, onOpenChange }) {
  const snapshots = useBannerStore(s => s.snapshots)
  const restoreSnapshot = useBannerStore(s => s.restoreSnapshot)
  const deleteSnapshot = useBannerStore(s => s.deleteSnapshot)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Restore a previous version of your banner.</DialogDescription>
        </DialogHeader>
