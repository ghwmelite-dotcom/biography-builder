...
export default function BookletVersionsDialog({ open, onOpenChange }) {
  const snapshots = useBookletStore(s => s.snapshots)
  const restoreSnapshot = useBookletStore(s => s.restoreSnapshot)
  const deleteSnapshot = useBookletStore(s => s.deleteSnapshot)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Restore a previous version of your programme booklet.</DialogDescription>
        </DialogHeader>
