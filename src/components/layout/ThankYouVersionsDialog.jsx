...
export default function ThankYouVersionsDialog({ open, onOpenChange }) {
  const snapshots = useThankYouStore(s => s.snapshots)
  const restoreSnapshot = useThankYouStore(s => s.restoreSnapshot)
  const deleteSnapshot = useThankYouStore(s => s.deleteSnapshot)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Restore a previous version of your thank you card.</DialogDescription>
        </DialogHeader>
