...
import BannerThemeForm from '../banner-editor/BannerThemeForm'

const sections = [
  { key: 'basic', title: 'Basic Information', icon: '1', component: BannerBasicForm },
  { key: 'content', title: 'Content & Scripture', icon: '2', component: BannerContentForm },
  { key: 'theme', title: 'Theme & Layout', icon: '3', component: BannerThemeForm },
]
...
function checkSectionComplete(key, state) {
  switch (key) {
    ...
    case 'content':
      return !!(state.scriptureVerse?.trim() || state.familyText?.trim())
    case 'theme':
      return !!state.bannerTheme
    default:
      return false
  }
}
...
function BannerBackupReminder() {
  const editCount = useBannerStore(s => s.editCountSinceLastSave)
  ...
  return (
    <div className="mb-3 px-3 py-2 bg-primary/10 border border-primary/20 rounded-lg flex items-center gap-2 text-xs text-primary">
      ...
      <button onClick={() => { saveBanner(); setDismissed(true) }} className="px-3 py-2 bg-primary text-white rounded text-xs hover:bg-primary/90">Save Now</button>
...
function useBannerAutoSave(intervalMs = 30000) {
  const saveBanner = useBannerStore(s => s.saveBanner)
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      if (useBannerStore.getState().isDirty) {
        saveBanner()
      }
    }, intervalMs)
...
export default function BannerEditorLayout() {
  ...
  useEffect(() => {
    const unsub = useBannerStore.subscribe(() => {
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="w-full lg:w-[420px] xl:w-[460px] border-r border-border overflow-y-auto overflow-x-hidden min-w-0 bg-background">
        <div className="p-4">
          <h2 className="text-xs text-muted-foreground uppercase tracking-wider mb-3 font-medium">Banner Editor</h2>

          <ProgressBar />
          <BannerBackupReminder />
        ...
        <div className="flex-1 flex flex-col bg-card min-h-0">
          <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-background shrink-0">
            ...
            <div className="flex items-center gap-2">
              <OrderPrintsButton
                designId={useBannerStore.getState().currentId}
                productType="banner"
                designName={pdfData.fullName ? `${pdfData.fullName} Banner` : 'Banner'}
                getDesignSnapshot={() => pdfData}
              />
              <GatedDownloadButton
                document={<BannerDocument data={pdfData} />}
                fileName={`${pdfData.fullName?.replace(/\s+/g, '-') || 'Memorial'}-Banner.pdf`}
                designId={useBannerStore.getState().currentId}
                productType="banner"
              />
            </div>
