...
export default function PosterNavbar() {
  ...
  const handleSave = () => {
    store.savePoster()
    notify('Poster saved successfully!', 'success')
    setShowSaved(true)
  ...
  const handleExport = () => {
    ...
    notify('Poster exported as JSON', 'success')
  }
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!data) {
        notify('Invalid poster data file.', 'error')
        return
      }
  ...
  const handleImportConfirm = () => {
    if (pendingImportData.current) {
      ...
      notify('Poster data imported successfully!', 'success')
    }
  }
