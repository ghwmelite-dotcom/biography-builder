...
import { BookOpenCheck, Undo2, Redo2, Save, Download, Upload, History, Sun, Moon } from 'lucide-react'
...
import BookletVersionsDialog from './BookletVersionsDialog'
import ImportConfirmDialog from './ImportConfirmDialog'

export default function BookletNavbar() {
  ...
  const isEditor = location.pathname.startsWith('/booklet')
  ...
  const handleSave = () => {
    store.saveBooklet()
    notify('Booklet saved successfully!', 'success')
    setShowSaved(true)
  ...
  const handleExport = () => {
    ...
    notify('Booklet exported as JSON', 'success')
  }
  ...
  const handleFileSelect = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!data) {
        notify('Invalid booklet data file.', 'error')
        return
      }
  ...
  const handleImportConfirm = () => {
    if (pendingImportData.current) {
      ...
      notify('Booklet data imported successfully!', 'success')
    }
  }
    ...
    <>
      <nav className="h-12 bg-background border-b border-border flex items-center justify-between px-4 shrink-0" role="navigation" aria-label="Main navigation">
        <Link to="/" className="flex items-center gap-2 text-card-foreground hover:text-foreground transition-colors" aria-label="Go to home page">
          ...
          <span className="text-sm font-semibold tracking-wide">Programme Builder</span>
        </Link>
      ...
      <BookletVersionsDialog open={versionsOpen} onOpenChange={setVersionsOpen} />
