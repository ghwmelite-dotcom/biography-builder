...
export default function InvitationVersionsDialog({ open, onOpenChange }) {
  const snapshots = useInvitationStore(s => s.snapshots)
  const restoreSnapshot = useInvitationStore(s => s.restoreSnapshot)
  const deleteSnapshot = useInvitationStore(s => s.deleteSnapshot)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Restore a previous version of your invitation.</DialogDescription>
        </DialogHeader>
