...
import { bannerTemplates } from '../../utils/bannerDefaultData'
import { BookOpen, ChevronDown } from 'lucide-react'

export default function BannerBasicForm() {
  ...
  const handleApplyTemplate = (key) => {
    const template = bannerTemplates[key]
    if (!template) return
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <input type="text" value={store.headerTitle} onChange={(e) => store.updateField('headerTitle', e.target.value)} placeholder="IN LOVING MEMORY OF" className={inputClass} />
        <p className="text-[10px] text-muted-foreground/60 mt-1">Appears at the top of the banner</p>
      </div>
